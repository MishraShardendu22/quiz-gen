...
4. storage.go design decision, create a hash for every document so that on every restart we can compare the current file with the stored version. If the hash is unchanged, skip re-chunking and re-embedding entirely. If the hash changes, delete the old chunks and regenerate them. If a new file is added, ingest it normally, and if a file is removed from the filesystem, delete its corresponding document and chunks from the database. This keeps the database in sync with the filesystem while avoiding unnecessary work.
